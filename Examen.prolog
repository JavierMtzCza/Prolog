...
calcular_rutas(A,B,Ruta-Tiempo):-   viajar(A,B,[],[A,B],Ruta-Tiempo1), 
                                    ...
                                    Tiempo #= Tiempo1 + Tiempo_inicial - 10.
...
viajar(A,B,Mem_lineas,_,[A,B]-Tiempo):-  sigue(A,B,Linea),
                                        valor_parámetro(tiempo_tramo, Tiempo_tramo), 
                                        valor_parámetro(tiempo_final, Tiempo_final),
                                        ( \+member(Linea,Mem_lineas) ->
                                            (transbordo(B,Grado) -> 
                                                Tiempo#=(Tiempo_tramo*Grado)+Tiempo_final+10 ; 
                                                Tiempo#=(Tiempo_tramo*2)+Tiempo_final+10
                                            ) 
                                            ;
                                            (transbordo(B,Grado) -> 
                                                Tiempo#=(Tiempo_tramo*Grado)+Tiempo_final ; 
                                                Tiempo#=(Tiempo_tramo*2)+Tiempo_final
                                            )              
                                        ).
...
%     reporte_viaje_simplificado( +estación1, +estación2).
%       
%       Explicación: Verdadero si la impresión en pantalla muestra la peor ruta para llegar de la <estación1> a la <estación2> de forma simplificada.
%                   Este predicado hace uso de imprimir_ruta_simplificado/2
%---------------
reporte_viaje_simplificado(A,B):- once(peor_ruta(A,B,Ruta,_)),
                                    nth1(2,Ruta,Segunda_estación),
                                    sigue(A,Segunda_estación,Línea),
                                    color(Línea,Color),
                                    format('~nComenzar en estación ~w de la linea ~w ~n~n',[A,Color]),
...
imprimir_ruta_simplificado(+Ruta).
        ...
        Explicación: Verdadero si la impresión en pantalla muestra la peor ruta para llegar de la <estación1> a la <estación2> de forma simplificada.
                    Este predicado utiliza dirección/4 para saber a que dirección se toma cada transbordo.
*/
imprimir_ruta_simplificado([X,Y]):- sigue(X,Y,Línea1),
                                    color(Línea1,Color1),
                                    dirección(X,Y,Línea1,Terminal),
                                    format('   Transbordar a la linea ~w, con dirección a ~w ~n   Hasta estación ~w. ~n~n',[Color1,Terminal,Y]).
...
/*
dirección/2. Imprimir la ruta en pantalla en lenguaje natural.
dirección(+estación1, +estación2, +linea, -Terminal).

        Explicación: Verdadero si <Terminal> es la dirección a la que se dirige <estación1> y <estación2> en <Linea>.
                    Este predicado hace uso de viaje_en_linea/5 de forma que sea mas fácil de usar para el usuario final. 
*/
dirección(A,B,Linea,Terminal):- viaje_en_linea(A,B,[A,B],Linea,Terminal).

/*
viaje_en_linea/5. Recorrer una linea desde una estación para encontrar la dirección a la que se dirige la ruta.
viaje_en_linea(+estación1, +estación2, +Mem_estaciones, +linea, -Terminal).

        Explicación: Verdadero si <Terminal> es la dirección a la que se dirige <estación1> y <estación2> en <Linea>.
        Forma de uso: viaje_en_linea(estación1,estación2,[estación1,estación2],linea,Terminal), debemos agregar <estación1> y <estación2> 
                        a <Mem_estaciones> para que no regrese y solo busque hacia la dirección que siguen estas estaciones en combinación.
*/
viaje_en_linea(_,A,_,Linea,A):- trayecto(Linea,A,_),!.
viaje_en_linea(_,A,_,Linea,A):- trayecto(Linea,_,A),!.
viaje_en_linea(A,B,Mem_estaciones,Linea,Terminal):- sigue(A,B,Linea),
                                                    sigue(B,Z,Linea),
                                                    \+member(Z,Mem_estaciones),
                                                    viaje_en_linea(B,Z,[Z|Mem_estaciones],Linea,Terminal).
...
trayecto(línea_3, universidad,indios_verdes).
...
trayecto(línea_B, ciudad_azteca,buenavista).
